...
+function($){
	...
	ChineseRegion.prototype.onSubmit = function(e) {
		...
		if(this.submitType == 'id'){
			if(this.areas[this.areas.length-1] != null){
	...
	ChineseRegion.prototype.renderAreasPanel = function(data,first) {
		...
		if(this.areas != undefined &&this.areas !="" ){
			...
			for (var i = 0; i < this.areas.length; i++) {
				this.$input.val(this.$input.val()+this.areas[i].name+'');//在input中拼字符串的
	 ...
	 */
	ChineseRegion.prototype.renderAreasPanel2 = function(data,index) {
		var $panel,$navbarButton;
			$panel = this.$tabContent.children('.active');
			$navbarButton = this.$el.find('.nav-tabs li.active');
		if(data && data.length){
			$panel.empty();
			for (var i = 0; i < data.length; i++) {
				var area = data[i];
		...
		if(data && data.length == 0 && this.$tabContent.children().length != index){
			$panel.html('--');
		}
		$navbarButton.children('a').tab('show');
		this.$input.val('');
	...
	/**
	 * 重绘（不刷新tab）
	 * @param minLevel 地区加载最小级别
	 */
	ChineseRegion.prototype.render2 = function(id,minLevel) {
		this.areas = this.getCurrentAreas(id);
		var index = parseInt(this.areas[this.areas.length-1].level) - parseInt(minLevel) + 1;
		this.renderAreasPanel2(this.getAreasByParentId(this.areas[this.areas.length-1].id),index);
	 ...
	 * @param minLevel 地区加载最小级别
	 */
	ChineseRegion.prototype.source2 = function(data,filter,minLevel) {
		if(arguments.length<=0) return this._source;
		var datas = typeof filter == 'function'?filter(data):data;
		var flag = true;
		if (datas.length > 0){
			for(var i=0; i<this._source.length;i++){
				if(this._source[i].id == datas[0].id)
					flag = false;
			}
		}
		if(flag)
			this._source = this._source.concat(datas);
		this.render2(this.areas[this.areas.length-1].id,minLevel);
	};
