...
+function ($) {
    ...
    Tablefixed.prototype.init = function() {
        ...
        this.options.styles = []
        this.$element.wrap('<div class="bjui-tablefixed clearfix" style="overflow: hidden"></div>')
    ...
    Tablefixed.prototype.initHead = function() {
        ...
        $fThs.each(function(i) {
            ...
            for (var k = 0; k < colspan; k++) {
                ...
                /* start 前两列 或 一列 固定宽度   add by xnn 20160722*/
                if (i==0 && $th.text() == '') w = ' width="30px"'
                if (i==0 && $th.text() == 'NO.') w = ' width="30px"'
                if (i==1 && $th.text() == 'NO.') w = ' width="30px"'
                /* end 前两列 或 一列 固定宽度*/
        ...
        var minW = ''
        if (this.$element.hasClass('min-width'))  minW=' min-width'
        $thead.wrap('<div class="fixedtableHeader" style="width:'+ (this.options.newWidth - Tablefixed.SCROLLW) + 'px;<!--overflow:hidden; xnn注释 20160721-->"><div class="fixedtableThead"><table class="table table-bordered'+minW+'" style="width:'+ (this.options.newWidth - Tablefixed.SCROLLW) + 'px; max-width:'+ (this.options.newWidth - Tablefixed.SCROLLW) +'px;"></table></div></div>')
                            ...
                            .basedrag({
                                ...
                                stop:  function() {
                                    ...
                                    if ((tableW + move + Tablefixed.SCROLLW) < that.options.newWidth) {
                                        ...
                                        //$fixed.find('.fixedtableHeader').width(newW - Tablefixed.SCROLLW)   2016-07-04 by xnn 显示有些问题
                                        $fixed.find('.fixedtableHeader').width(newW)
            ...
            $('div.bjui-resizeGrid').each(function() {
                ...
                if (width) {
                    $this.find('.bjui-tablefixed').each(function() {
                        ...
                        //$fixed.find('.fixedtableHeader').width(realWidth - Tablefixed.SCROLLW)   2016-07-04 by xnn 显示有些问题
                        $fixed.find('.fixedtableHeader').width(realWidth)
    ...
    /* selected tr */
    $(document).on('click.bjui.tr.data-api', 'tr', function(e) {
        var $this     = $(this),
            $table    = $this.closest('table'),
            multi     = $table.data('selectedMulti'),
            $chkbox=$this.find('[data-toggle="icheck"]').eq(0),
           // icheckVal        = $chkbox.val(),
            clsName   = 'selected',
            $selected = $table.closest('.unitBox').find('#bjui-selected'),
            $form=$table.closest('.unitBox').find('form');



       // if(($form.attr("id").toLowerCase()=="pagerform")||($table.attr('data-toggle')=='datagrid'))
        if(!$table.parents('div').hasClass('editbox'))
        $this.toggleClass(clsName)

        $this.hasClass(clsName)?$chkbox.iCheck('check'):$chkbox.iCheck('uncheck');
        if (multi) {
           // icheckVal = []
            $this.siblings('.'+ clsName).add(($this.hasClass(clsName) ? $this : '')).each(function() {
            // $chkbox=$(this).find('[data-toggle="icheck"]');
             //$chkbox.iCheck('check');
             //icheckVal.push($chkbox.val())
            })
           // icheckVal = icheckVal.join(',')
        } else {
            $this.siblings().removeClass(clsName).find('[data-toggle="icheck"]').iCheck('uncheck')
            //if (!$this.hasClass(clsName)) icheckVal = ''
        }
//        if ($selected && $selected.length) {
//            $selected.val(icheckVal)
//        } else {
//            $selected = $('<input type="hidden" id="bjui-selected" value="'+ icheckVal +'">')
