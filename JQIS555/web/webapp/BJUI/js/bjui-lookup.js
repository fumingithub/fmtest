        ...
        if ($currentLookup.data('customEvent')) {
            ...
            $box.find(':input').each(function() {
                var $input = $(this), inputName = $input.attr('name'),
                	inputType=$input.attr('type'),inputTagName=$($input)[0].tagName,
                ...
                for (var key in args) {
                    ...
                    if(typeof inputName =="undefined") break;
                    
                    if (name == inputName || key == inputName) {
                        ...
                        if(inputTagName=="SELECT") {
                        	$input
	                        	.selectpicker('val', newValue) /* @description 修改 args[key] 为 newValue @author 小策一喋 */
	                        	.trigger(Lookup.EVENTS.afterChange, {value:args[key]})
                        	
                        	break
                        } else {
                        	if(inputType == "radio") {
                        		$("input[name="+inputName+"][value="+newValue+"]").attr("checked",true).trigger(Lookup.EVENTS.afterChange, {value:args[key]});
                        		break;
                        	}
                        	if(inputType == "checkbox") {//eg.args=['a','b']
                        		for(var i=0;i<newValue.length;i++) {
                        			$("input[name="+inputName+"]").each(function(){
                        				if(this.value==newValue[i]) {
                        					this.checked=true;
                        				}
                        			})
                        		}
