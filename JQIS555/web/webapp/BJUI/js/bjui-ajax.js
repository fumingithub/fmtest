...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxError = function(xhr, ajaxOptions, thrownError) {
        ...
        if (msg.startsWith('{')) {
          ...
          /*  this.$element.alertmsg('error', '<div>Http status: ' + xhr.status + ' ' + xhr.statusText + '</div>'
            ...
            + '<div>'+ msg +'</div>')*/
        	this.$element.alertmsg('Http status:', xhr.status)
        	if(xhr.status=="403"){
        		this.$element.alertmsg('error', '<div>对不起您没有权限，请联系管理员</div>')
        	}else{
        		this.$element.alertmsg('error', '<div>操作失败，请联系管理员</div>')
        	}
    ...
    Bjuiajax.prototype.divCallback = function(json, $target) {
        ...
        if (json.forward) {
            var _forward = function() {
                var urlPar=json.forward.getRequest();
                if(urlPar.length>0){
                    $.each(urlPar,function(e){
                        url=url.setUrlParam(urlPar[e].key,urlPar[e].value)
                    })
                }
                $target.ajaxUrl({url: url})
            }
    ...
    Bjuiajax.prototype.navtabCallback = function(json) {
        ...
        if (json.forward) {
            var _forward = function() {
                var urlPar=json.forward.getRequest();
                var url=$.CurrentNavtabLi.data("options").url;
                if(urlPar.length>0){
                    $.each(urlPar,function(e){
                        url=url.setUrlParam(urlPar[e].key,urlPar[e].value)
                    })
                }
                that.$element.navtab('reload', {url:url})
            }
    ...
    Bjuiajax.prototype.dialogCallback = function(json) {
        ...
        if (json.forward) {
            var _forward = function() {
                var url= $.CurrentDialog.data('options').url
                var urlPar=json.forward.getRequest();
                if(urlPar.length>0){
                    $.each(urlPar,function(e){
                        url=url.setUrlParam(urlPar[e].key,urlPar[e].value)
                ...
                that.$element.dialog('reload', {url:url})
            }
    ...
    Bjuiajax.prototype.doSearch = function(options) {
    	var start;
    	if(options.refreshCurrentPage || options.refreshCurrentPage == "true"){
    		start = this.$element.find("input[name='start']").val();
    	}else{
    		start=1;
    	}
        var that = this, $element = that.$element, form = null, op = {pageCurrent:start}, $target = $element.closest('.bjui-layout'), isValid = options.isValid
    ...
    Bjuiajax.prototype.doExport = function(options) {
        ...
        var callbackfuc = function() {
         ...
         // 下载失败回调函数
        var errorCallbackfuc = function(data) {
            var errorcallback = options && options.errorcallback;
            if (errorcallback) errorcallback = errorcallback.toFunc();
    ...
    $(document).on('click.bjui.bjuiajax.data-api', '[data-toggle="reloadsearch"]', function(e) {
        ...
        options.refreshCurrentPage = $this.attr('data-refreshCurrentPage');
        $this.attr('data-refreshCurrentPage','');
        Plugin.call($form, 'doSearch', options)
    ...
    $(document).on('click.bjui.bjuiajax.data-api', '[data-toggle="doexport"]', function(e) {
        ...
        if (!options.url||options.url.isFinishedTm()) options.url = $this.attr('href')
