...
+function ($) {
    ...
    var Datepicker = function(element, options) {
        ...
        this.$dateBtn = null
        this.$utbox=this.$element.closest('.unitBox')
        var $minBox=this.$utbox.find(this.options.minDate),$maxBox=this.$utbox.find(this.options.maxDate)
        //动态minDate、maxDate
        var now = new Date()

        if($minBox.length==1&&$minBox.is(':text'))
            this.options.minDate=$minBox[0].value==''?Datepicker.DEFAULTS.minDate:$minBox[0].value
        if(now.IsValidDate(this.options.minDate))
            this.options.minDate=now.formatDateTm(this.options.minDate)
        else
            this.options.minDate=now.formatDateTm(Datepicker.DEFAULTS.minDate)

        if($maxBox.length==1&&$maxBox.is(':text'))
            this.options.maxDate=$maxBox[0].value==''?Datepicker.DEFAULTS.maxDate:$maxBox[0].value
        else if(now.IsValidDate(this.options.maxDate))
            this.options.maxDate=now.formatDateTm(this.options.maxDate)
        else
            this.options.maxDate=now.formatDateTm(Datepicker.DEFAULTS.maxDate)




        //this.options.minDate = now.formatDateTm(this.$utbox.find(this.options.minDate).val())
        //this.options.maxDate = now.formatDateTm(this.options.maxDate)
    ...
    Datepicker.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            clickTmMenu: function($input, type) {
                $tm
                    ...
                    .on(that.events.click_tm, function() {
                        ...
                        $input.val(val).focus().trigger("validate")
                    })
            },
            ...
            changeTm: function($input, $btn) {
                ...
                $input.val(newVal).focus().trigger("validate")
            },
    ...
    Datepicker.prototype.addBtn = function() {
        ...
        if (!this.$dateBtn && !this.options.addbtn && !$element.parent().hasClass('wrap_bjui_btn_box')) {
            ...
            this.$dateBtn.css({'margin-top': '4px'}).appendTo($box)  /*'height':height, 'lineHeight':height +'px'*/
    ...
    Datepicker.prototype.init = function() {
        ...
        $clearBtn.off(this.events.click_clear).on(this.events.click_clear, function() {
            that.$element.val('').focus().trigger("validate")
        ...
        $okBtn.off(this.events.click_ok).on(this.events.click_ok, function() {
            ...
            tools.closeCalendar(true)
            that.$element.val(tools.formatDate(date)).focus().trigger("validate")
    ...
    function Plugin(option) {
        ...
        return this.each(function () {
            ...
            $this.data('bjui.datepicker', (new Datepicker(this, options)))
