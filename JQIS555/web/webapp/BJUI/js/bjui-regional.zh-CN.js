...
+function ($) {
    ...
    $(function() {
        ...
        /* 占位符对应选择器无有效值提示 */
        BJUI.setRegional('plhmsg', '请选择一条数据！')
        ...
        $.validator && $.validator.config({
            ...
            rules: {
                ...
                ,mobilePhone:[/^(1[3-9]\d{9})$|^(?:(?:0\d{2,3}[\- ]?[1-9]\d{6,7})|(?:[48]00[\- ]?[1-9]\d{6}))$/,'请输入正确的手机或固话号码']
                ,phone:[/(^((\d{11})|(\d{7,8})|(\d{4}|\d{3})-(\d{7,8}))$)/,'请输入正确的手机或固话号码']
                ...
                ,ID_card: function test(e) {
                    ...
                    return true;
                }//[/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[A-Z])$/, '请输入正确的身份证号码']
                ,url: [/^(https?|ftp):\/\/[^\s]+$/i, '网址格式不正确']
                ,postcode: [/^[1-9]\d{5}$/, '邮政编码格式不正确']
                ,chinese: [/^[\u0391-\uFFE5]+$/, '请输入中文']
                ,username: [/^\w{3,12}$/, '请输入3-12位数字、字母、下划线']
                ,code: [/^[-\w]*$/, '请输入数字、字母、下划线（_）、连接符（-）']
                ...
                float1: [/^\-?([1-9]\d*(\.\d{1})?|0\.\d{1})$|^0$/,'请填写最多一位小数的有效数字'],
                float2: [/^\-?([1-9]\d*(\.\d{1,2})?|0\.\d{1,2})$|^0$/,'请填写最多两位小数的有效数字'],
                float3: [/^\-?([1-9]\d*(\.\d{1,3})?|0\.\d{1,3})$|^0$/,'请填写最多三位小数的有效数字'],
                float4: [/^\-?([1-9]\d*(\.\d{1,4})?|0\.\d{1,4})$|^0$/,'请填写最多四位小数的有效数字']
