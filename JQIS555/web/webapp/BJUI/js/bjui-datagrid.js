...
+function ($) {
    ...
    Datagrid.DEFAULTS = {
        ...
        columnResize: false,     // Allow adjust the column width
        columnMenu: false,     // Display the menu button on the column
        columnShowhide: false,     // On the menu display (show / hide columns)
        columnFilter: false,     // On the menu display (filter form)
        columnLock: false,     // On the menu display (lock / unlock columns)
        paging: false,     // Display pagination component
        pagingAlign: 'center', // The pagination component alignment
        editUrl: null,     // An action URL, for processing (update / save),
        iscallbyajax: true,// return results (json)
        ...
        contextMenuH: false,     // Right-click on the thead, display the context menu
    ...
    Datagrid.prototype.TOOLS = function () {
        ...
        var tools = {
                    ...
                    if (type == 'array' && data && data.length && $.type(data[0]) == 'array') {
                        ...
                        for (var i = a1; i < a2; i++) {
                            ...
                            $.each(allData[i], function (k, v) {
                                ...
                                if (allData[i].rowstate == 'delete') {

                                } else {
                                    if (allData[i].rowstate == 'create') {
                                    }
                                    else
                                        allData[i].rowstate = 'update';
                                    if (model[_index] && model[_index].gridNumber)   _index++
                                    if (model[_index] && model[_index].gridCheckbox) _index++
                                    if (typeof val == 'string') val = '"' + val + '"'
                                    else if (val instanceof Date) val = val.pattern(model.pattern);
            ...
            //格式化 data中日期格式
            objToDate:function(data)
            {
                var tools = this, model = that.columnModel,allData=that.allData
                if(data)
                $.each(data,function(i,trData){
                    $.each(model,function(j,mi){
                       var s= trData[mi.name]

                        if(s){
                        if (typeof s == 'object') {
                            if (new Date(s.time) instanceof Date) {
                                var t = new Date(s.time);
                                s = t.Format(mi.pattern);
                            }
                        }
                        trData[mi.name]=s;
                        }
            ...
            createTrs: function (datas, refreshFlag) {
                ...
                $.each(datas, function (i, trData) {
                    ...
                    $.each(model, function (j, n) {
                        ...
                        //if(n.notshow)
                        //{
                        //    label='<div><input type="hidden"  name="datagrid.hidden" value=""></div>';
                        //}
                        ...
                        //if(n.notshow)
                        //{
                        //    label.appendTo($tr.find('td').eq(0))
                        //}else{
            ...
            loadItems: function () {
                ...
                if (that.columnModel) {
                    $.each(that.columnModel, function (k, n) {
                        if (typeof n.items == 'function') {
                            $.when(n.items.call(that)).then(function (item) {
                                n.items = item
                            })
            ...
            createThead: function () {
                var tools = this, columns = options.columns, rowArr = [], rows = [], label, align, width, colspan, rowspan, resize, menu, notshow
                        ...
                        if (op.type) {
                            switch (op.type) {
                                ...
                                case 'select':
                                    ...
                                    that.inputs.push('<select name="' + name + '" data-toggle="selectpicker"' + rule + ' data-emptytxt="--请选择--" data-width="100%">' + selectoptions.join('') + '</select>')
                                ...
                                case 'boolean':
                                    ...
                                    break
                                case 'lookup':
                                    that.inputs.push('<input type="text" name="' + name + '" data-isrefshpar="false" data-toggle="lookup" data-custom-event="true"' + rule + attrs + '>')
        ...
        if ($trs.length && trLen == 1) {
            ...
            $trs.find('> th').each(function (i) {
                ...
                if (that.columnModel.length && that.columnModel[i]) {
                    ...
                    // op.notshow = (typeof op.notshow == 'undefined') ? false  : true
    ...
    Datagrid.prototype.init = function () {
        ...
        //that.$element.appendTo(that.$grid);
        //that.$grid.append(that.$element);
        ...
        if (that.isDom) {
            ...
            that.$tbody = null
            if (options.data || options.dataUrl) tools.createTbody(), that.initThead()
        }
    }
    ...
    Datagrid.prototype.initThead = function () {
        ...
        if (that.$boxH) {
            that.$grid.append(that.$boxH)
            if (that.$boxH.find('.datagrid-wrap-h').length != 0)
                that.$boxH.empty();
            that.$boxH.append($('<div class="datagrid-wrap-h"></div>').append(that.$tableH))
    ...
    Datagrid.prototype.initEvents = function ($trs) {
        ...
        $trs
            ...
            .on('bjui.datagrid.tr.delete', function (e) {
                ...
                //if(!(data instanceof jQuery)||!(allData instanceof jQuery))
                //{
                //    data=$(data)
                //    allData=$(allData)
                //}
                ...
                if (!that.isDom) {
                    ...
                    if (data[index].rowstate == 'create')
                        $(data).remove(index), $(allData).remove(gridIndex);
                    else
                        data[index].rowstate = 'delete', allData[gridIndex].rowstate = 'delete';

                    that.data = data;        // remove data in the current page data
    ...
    Datagrid.prototype.colShowhide = function (showFlag) {
        var that = this, options = that.options, tools = that.tools, $menu = that.$menu, $ul, $showhideli;
        if ($menu)$ul = $menu.find('> ul')
        if ($ul)  $showhideli = $ul.find('> li.' + that.classnames.li_showhide)
        if (showFlag) {
            if (!that.$showhide) {
                tools.createShowhide()
                if ($showhideli && $menu)
    ...
    Datagrid.prototype.initPaging = function () {
        ...
        $.each(selectPages, function (i, n) {
            ...
            if (parseInt(n) == paging.pageSize) $option.prop('selected', true)
    ...
    Datagrid.prototype.add = function (addLocation) {
        ...
        if ((!this.options.editUrl) && (this.options.iscallbyajax)) {
        ...
        that.emptyData.rowstate = 'create';
    ...
    Datagrid.prototype.doEditRow = function (row, type, isAdd) {
        ...
        if ((!that.options.editUrl) && (that.options.iscallbyajax)) {
            BJUI.debug('Datagrid Plugin: Edit url is not set!')
            return
        }
    ...
    Datagrid.prototype.delRows = function (rows) {
        ...
        var delEnd = function () {
            ...
            if (options.tohid) {
                var hidobj = $("#" + options.tohid);
                if (hidobj.length > 0) {
                    delete that.allData.gridCheckbox
                    delete that.allData.gridEdit
                    hidobj.val(JSON.stringify(that.data))
                }
            }
        }
            ...
            $tr.isValid(function (v) {
                if (v) {
                    ...
                    if (options.iscallbyajax)
                        $tr.bjuiajax('doAjax', {
                            url: options.editUrl,
                            data: {json: JSON.stringify(postData.push(tempData) && postData)},
                            type: 'POST',
                            callback: callback
                        })
                    else
                        callback(JSON.stringify(postData.push(tempData) && postData)), that.$grid.dialog('close', 'datagrid-dialog-edit');
                }
            })
        }
    }
    ...
    Datagrid.prototype.dialogEdit = function ($tr, isAdd) {
        ...
        var onLoad = function ($dialog) {
            ...
            var createForm = function (data, $form, $tr) {
                ...
                var $table = $("<table clase='table table-bordered'></table>");
                var $ctr = $("<tr></tr>")
                $.each(columnModel, function (i, n) {
                    if (n == that.linenumberColumn || n == that.checkboxColumn || n == that.editBtnsColumn) return true
                    ...
                    if ($input.isTag('select')) $input.attr('data-width', '180')
                    else if (!$input.isTag('checkbox')) $input.attr('size', 18)

                    if (n.type == 'boolean') {
                        $input.attr('checked', data[n.name])
                    } else if (n.type == 'select') {
                        $input.attr('data-val', data[n.name])
                        $input.find('option').each(function (i) {
                            if ($(this).val() == data[n.name])$(this).attr("selected", "selected")
                        });
                    }
                    else {
            ...
            $save.click(function () {
                ...
                $form.isValid(function (v) {
                    if (v) {
                        ...
                        if (callback) {
                            ...
                            callback = function (json) {
                                ...
                                if (options.tohid) {
                                    var hidobj = $("#" + options.tohid);
                                    if (hidobj.length > 0) {
                                        //var tdata=that.allData;
                                        //delete tdata.gridCheckbox
                                        //delete tdata.gridEdit
                                        //delete that.data.statusCode
                                        //修改datagrid bug 不能正常保存到hidden元素中信息

                                         that.data[$tr.index()]=data;
                                        hidobj.val(JSON.stringify(that.data))
                                    }
                                }
                            }
                        ...
                        //callback(JSON.stringify(postData)),that.tools.initTbody(zhdata),that.$grid.dialog('close', 'datagrid-dialog-edit');
                    }
                })
            })
