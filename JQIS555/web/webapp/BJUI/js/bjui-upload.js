...
+function ($) {
    ...
    Upload.DEFAULTS = {
        ...
        onAllUploadComplete : null,  //上传所有完成的动作
    ...
    Upload.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            renderQueueItem: function(file) {
                ...
                if(options.multi)
                {
                    $temp.find('> .info > .up_confirm').hide()
                }
...
//                if(url.indexOf("?")!=-1){
//                    url +="&fname="+encodeURIComponent(encodeURIComponent(options.formData.fileName));
//                }else{
//                    url +="?fname="+encodeURIComponent(encodeURIComponent(options.formData.fileName));
//                }
                ...
                fd.append("fname",options.formData.fileName);
                xhr.send(fd)

            },
            //发送文件块函数
            sendBlobAll: function(url, xhr, files, formdata) {
                var fd = new FormData()

//encodeURIComponent
//                if(url.indexOf("?")!=-1){
//                    url +="&fnames="+JSON.stringify(formdata.fileNames)+"&count="+files.length;
//                }else{
//                    url +="?fnames="+JSON.stringify(formdata.fileNames)+"&count="+files.length;
//                }
                xhr.open(options.method, url, true)
                xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest')
                //xhr.setRequestHeader('X_Requested_With', location.href.split('/')[5].replace(/[^a-z]+/g, '$'))
                //files=that.$element.find('.bjui-upload-select-file')[0].files
                for(var i=0;i<files.length;i++)
                {
                    fd.append("files", files[i]);
                    fd.append("fnames",formdata.fileNames[i]);
                }
                if (formdata) {
                    for (var key in formdata) {
                        fd.append(key, formdata[key])
    ...
    Upload.prototype.init = function() {
        ...
        var $uploadbtn;
        if (options.uploadallbtn)
        {
            var tid="#"+$element.attr("id");
            $uploadbtn=$("<a class='btn  btn-default bjui-upload-all' >开始上传</a>");

        }
        var $gpy;
        if(options.gpy) {
            var ctxUrl = options.ctx;
            $gpy= $("<a class='btn btn-default' data-title='高拍仪' data-toggle='dialog'"
                + "data-url='" + ctxUrl + "/admin/gaopaiyi!edit.do'"
                + "data-before-close='beforeClose'"
                + "data-id='gaopaiyi_page' data-mask='true' data-width='1000' data-height='660'>高拍仪</a>");
        ...
        this.$uploadFileList.before($uploadbtn);
        this.$uploadFileList.before($gpy);
        //do select files
        $element
            .on('click.bjui.upload','.bjui-upload-all',function(e){
                that.fileUploadAll();
            })
    ...
    Upload.prototype.fileUploadAll = function(files, uploadedSize) {
        var that = this, $element = that.$element, options = that.options, tools = that.tools



        if(that.files==undefined||that.files==[]||that.files==""){
        	return;
        }
        if(!files) files=that.files?that.files:$element.find('.bjui-upload-select-file')[0].files;


        //$element.find('.up_confirm').trigger('click')
        var xhr  = false, originalFile =[];
        if(!uploadedSize) uploadedSize=options.uploadSize;
        //校正进度条和上传比例的误差
        xhr = new XMLHttpRequest()
        options.formData.fileNames=[];
        options.formData.lastModifiedDates=[]
        options.formData.fileTypes=[]
        $.each(files,function(i,k){
            if (options.breakPoints) {
                //对文件进行切割，并保留原来的信息
                originalFile.push(k.slice(uploadedSize, uploadedSize + options.fileSplitSize))
            }
            if(options.formData.fileNames)
            options.formData.fileNames.push(k.name)
            if(options.formData.lastModifiedDates)
            options.formData.lastModifiedDates.push(k.lastModifiedDate.getTime())
            if(options.formData.fileTypes)
            options.formData.fileTypes.push(k.type)
        })
        if (xhr.upload) {
            // 上传进度
            xhr.upload.onprogress = function(e) {
                $.each(files,function(i,k) {
                    that.onProgress(k, e.loaded, k.size)
                })
            }
            // 上传回调
            xhr.onreadystatechange = function(e) {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    $.each(files, function (i, k) {
                        var returnData = JSON.parse(xhr.responseText), upOver = false, allOver = false;
                        if (options.breakPoints) {
                            //更新已上传文件大小，保存到本地
                            uploadedSize += options.fileSplitSize
                            tools.saveUploadedSize(k.id, uploadedSize)
                            //继续上传其他片段
                            if (uploadedSize < k.size) {
                                files = k.slice(uploadedSize, uploadedSize + options.fileSplitSize)
                                //上传文件
                                tools.sendBlob(options.uploader, xhr, files, options.formData)
                            } else {
                                upOver = true
                            }
                        } else {
                        ...
                        if (upOver) {
                            ...
                            tools.successQueueItem(k, xhr)
        
                            //$element.find('.bjui-upload-select-file')[0].files.splice(k);
                            options.onUploadComplete && options.onUploadComplete(k, xhr.responseText)
                        }
                    })
                    if(options.onAllUploadComplete)//
                    {
                        if (that.queueData.files = that.queueData.success+that.queueData.error) {
                            options.onAllUploadComplete && options.onAllUploadComplete.toFunc().call(that, files, xhr.responseText, that.$element)
                        }
                    }
                    ...
                    //错误回调
                    $.each(files, function (i, k) {
                        options.onUploadError && options.onUploadError(k, xhr.responseText)
                    })
                }
               //var $queue = $element.find('.item')
               // $.each($queue,function(i,k){
               //     var a=k.data('upfile');
               //     var b=k.data()
               //
               //
               // })
                ...
                if (options.multi)
                {
                    //if(options.onAllUploadComplete)//
                    //{
                    //    if (that.queueData.files = that.queueData.success)
                    //        options.onAllUploadComplete && options.onAllUploadComplete.toFunc().call(that, files, xhr.responseText, that.$element)
                    //}
                }else
                {
            ...
            tools.sendBlobAll(options.uploader, xhr, files, options.formData)
            var that = this
            that.files=[];
        }
    ...
    function Plugin(option) {
        ...
        if (window.FileReader) {
            return this.each(function () {
                ...
                //if (!data)
                	$this.data('bjui.upload', (data = new Upload(this, options)))
            ...
            this.each(function() {
                ...
                if (!(options.uploader)) {
                    ...
                    options.uploader = decodeURI(options.uploader).replacePlh($element.closest('.unitBox')).setUrlParam("ietype","ie")
                ...
                options.onUploadSuccess = function(file, data, response){
                	pic_upload_success(file,data);
                	//stringtoFunc(options.onAllUploadComplete,file,data);
                }
