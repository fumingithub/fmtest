...
+function ($) {
    ...
    var currentIndex, $currentTab, $currentPanel, $box, $tabs, $panels, $prevBtn, $nextBtn, $moreBtn, $moreBox, $main, $mainLi,$parentIndex
    ...
    $(function() {
        var INIT_NAVTAB = function() {
            ...
            $moreBtn.click(function() { $moreBox.toggle() })
    ...
    Navtab.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            closeTab: function(index, openTabid) {
                ...
                if($form.length>1){
                    $form = $panel.find('form.active');
                }
                if($form.data("toggle")=="validate"&&(!$form.data("cclose"))&&(!$form.data().noCheck))
                {

                    $form.alertmsg("confirm","您修改了当前表单信息，关闭该页面将丢失您修改的信息！<br/>请问是否继续",
                        {okCall:function(){ $form.data("cclose",true);
                            $.CurrentNavtab.navtab("closeTab",options.id)},
                            cancelCall:function(){
                                $.CurrentNavtab.navtab("switchTab",options.id);
                                return false;}}
                    )
                }else{
	                ...
	                //jyy
					//$parentIndex=$parentIndex>-1?$parentIndex:$tab.data('options').parentIndex;
				
	                if (onClose) onClose.apply(that)
	                if (currentIndex >= index) currentIndex--
	                if (!openTabid) {
	                    openTabid=options.parid
	                }
	                if(options.isrefsh&&openTabid&&this.getPanel(openTabid)){
	                    var openIndex = this.indexTabId(openTabid)
	                    if (openIndex > 0) currentIndex = openIndex
	                    $(this.getPanel(openTabid).find('form')).bjuiajax('doSearch', $(this.getPanel(openTabid).find('form')).data())
	                }
            ...
            loadUrlCallback: function($panel) {
				...
				$panel.find('.closecutab').click(function() {
            ...
            bindings: {
                ...
                closeOther: function(t, m) {
                    if (t.index() == 0) {
                        ...
                        that.tools.closeOtherTab(index > 0 ? index : currentIndex,t.data('initOptions').id)
                    }
                },
    ...
    Navtab.prototype.openTab = function() {
        ...
        if(typeof options.showtip != "undefined" 
        	&& typeof that.tools.getTab(options.id) != "undefined" 
        		&& options.showtip) {
        	// modify liuj showtip--要打开的页面已存在，是否提示
        	$(that).alertmsg("confirm","您将打开的页面已存在，关闭该页面将丢失您修改的信息！<br/>请问是否继续",
                {
	        		okCall:function(){
	        			openTT(that,$element,options,tools);
	    			},
                    cancelCall:function(){
                        $.CurrentNavtab.navtab("switchTab",options.id);
                        return false;
        	...
        	if (!(options.url)) {
                ...
                if(options.url.indexOf('?')>-1){
                    options.url=options.url+"&_tabid="+options.id;
                 }else{
                    options.url=options.url+"?_tabid="+options.id;}
                options.url = encodeURI(options.url);
            }
    		//jyy
            var iOldtabid=options.oldid?0:tools.indexTabId($currentTab.data('options').id);
    		$parentIndex=tools.indexTabId(iOldtabid);
    		options.parentIndex=$parentIndex;
    ...
    function openTT(that,$element,options,tools) {
    	if (!(options.url)) {
            BJUI.debug('Navtab Plugin: Error trying to open a navtab, url is undefined!')
            return
        } else {
    ...
    function Plugin(option) {
        ...
        return this.each(function () {
            ...
            // var panels= ;//获取所有panel
            var tabs= $("#bjui-navtab").find('.navtab-tab').find('>li');//获取所有tab
            var i=1;
            var pid='';
            if(options.isrefsh)
            $("#bjui-navtab").find('.navtab-tab').find('>li').each(function(index){
