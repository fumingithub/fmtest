...
+function ($) {
        ...
        if ($dialog) {
            ...
            if ($pagerForm && $pagerForm.length) {
                ...
                if (clearQuery) {
                    ...
                    // 点击“刷新”按钮，清空表单里的查询条件 add by jinliang at 2016-12-31
                    for (var item in options.data) {
                        if ("limit,start,orderField,orderDirection".indexOf(item) == -1) {
    ...
    Dialog.prototype.close = function(dialog) {
        ...
        //取消dialog页面信息修改的监听事件 需在页面添加“closedia”属性
        var cdialog =$form.data("closedia");
        if(null!=cdialog&&cdialog==true){
            $form.data("cclose",true);
        }
        ...
        if($form.data("toggle")=="validate"&&(!$form.data("cclose"))&&(!$form.data().noCheck))
        {
            ...
            $form.alertmsg("confirm","您修改了当前表单信息，关闭该页面将丢失您修改的信息！<br/>请问是否继续",
                {okCall:function(){
                    ...
                    cancelCall:function(){
                        $dialog.dialog("switchDialog",$dialog);
                        return false;}}
            )
        }else{
        ...
        $dialog.animate({top:-$dialog.outerHeight(), opacity:0.1}, 100, function() {
        ...
        var openid=options.parid
        var tabs=$("#bjui-navtab").find('.navtab-tab').find('>li');
        var panels=$("#bjui-navtab").find('.navtab-panel').find('>div');
        var openindex=0;
        tabs.each(function (i){ if($(this).data('options').id==openid)openindex=i;});
        if(options.isrefshpar)
        {

            if(options.partag=="dialog"){//如果父类框是dialog 则刷新当前
                // that.refresh($.CurrentDialog.data("options").id) //======原始版本代码
                that.refresh($.CurrentDialog.data("options").parid);
                setTimeout(function () {
                    if (options.partag == "dialog") { // 如果父类框是dialog
                        $.CurrentDialog = $current = $('body').data(openid);
                    }
                }, 200);
            }else{
                var $tabform=$(panels.eq(openindex)).find('form').eq(0)
                if($tabform.attr("id").toLowerCase()=="pagerform")
                {
                    $tabform.submit();
                }else
                {
                    $(tabs).navtab('refresh',openindex);
                }
            }
    ...
    function Plugin(option) {
        ...
        return this.each(function () {
            ...
            {
                pid= $.CurrentDialog.data("options").id;
                $this.data("partag","dialog")
            }else{
