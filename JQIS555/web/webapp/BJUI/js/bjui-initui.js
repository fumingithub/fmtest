...
+function ($) {
    ...
    $(document).on(BJUI.eventType.afterInitUI, function(e) {
        ...
        $box.resizePageH()
        //add by jyy
      //  $form.find('.make-switch').bootstrapSwitch();

        //form data
        if($form.length>0) {
            var ischange = true;
            if(!$form.data().noCheck){
	            $form.data("cclose", ischange);
	            //if($form.data("toggle")=="validate"){
	            $form.data("initform", $form.serializeJson());
	            $form.find(":text,:checkbox,:radio,:password,select,textarea").on("change", function (e) {
	                var field = $form.data("initform")[$(this).attr("name")];
	                if ($(this).val() != field) {
	                    ischange = false
	                } else {
	                    ischange = true;
	                ...
	                if ($(this).is(":checkbox")) {
	                    if (($(this).attr("checked") == "checked") && field == "on") ischange = true
	                    else if (($(this).attr("checked") != "checked") && field == "off") ischange = true
	                    else ischange = false;
	                }
	                $form.data("cclose", ischange);
	            })
        ...
        //submit
        if ($form.length == 1) {
            $box.find('> .bjui-pageFooter,> .bjui-pageHeader').find(':submit,.submit').on('click.bjui.submit', function(e) {
                e.preventDefault()
